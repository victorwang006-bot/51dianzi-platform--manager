...
## 用户需求（2026-07-30 第三十九轮）：物料数据库初始闭合状态
- [x] 梳理物料页当前展示结构，确定"闭合状态"实现方案（初始不铺开全部物料列表）
- [x] 实现：刚打开物料数据库页面为闭合状态（不发请求、显示搜索引导占位），搜索/筛选/点击"显示全部物料"后展示列表
- [x] 开发环境验证（截图确认闭合状态）+ 单元测试 57/57、tsc 通过
- [x] 部署生产后台并验证（dist.bak.r39 备份，pm2 重启，资源/接口 200），保存检查点推送 GitHub（db646ab → 0b8b52b）

## 用户需求（2026-07-30 第四十轮）：商户详情工商信息整合 + 客户物料管理
- [x] 排查前台商户物料发布数据结构与状态字段（inventories 表 status: draft/published/offshelf，下架回流=draft 待发布）
- [x] 商户详情页：营业执照图片整合到工商信息栏（点击链接查看大图），移除入驻资料独立大图展示
- [x] 商户详情页：法人信息（姓名/身份证号/联系电话）并入工商信息栏
...
- [x] 保存检查点推送 GitHub，部署生产并验证（GitHub 1f21e84，下架接口写入 offshelfBy='admin'+原因验证通过，数据已恢复）
