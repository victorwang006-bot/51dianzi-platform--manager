...
const legacyBuildVerifier = readFileSync(
...
describe("阶段 D 后台子域构建契约", () => {
  it("保留旧 /admin/ 构建并新增独立根路径构建", () => {
    expect(packageJson.scripts["build:admin"]).toContain(
      "VITE_BASE_PATH=/admin/"
    );
    expect(packageJson.scripts["verify:admin-build"]).toContain(
      "verify-admin-build.sh"
    );
    expect(packageJson.scripts["build:subdomain"]).toContain(
      "VITE_BASE_PATH=/"
    );
...
describe("阶段 D 后台子域 Nginx 契约", () => {
  it("为精确后台主机名使用独立证书和独立静态制品", () => {
    ...
    expect(subdomainNginx).not.toContain(
      "ssl_certificate /etc/nginx/ssl/51dianzi.com.pem;"
    );
  });
  ...
  it("HTTP 只重定向 HTTPS，且灰度期禁止索引而不设置 HSTS", () => {
    ...
    expect(subdomainNginx.toLowerCase()).not.toContain(
