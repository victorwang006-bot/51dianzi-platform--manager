...
function OrderStatus({ status }: { status: string }) {
  ...
  if (!meta) {
    return (
      <Badge className={UNKNOWN_STATUS_CLASS}>{status || "未知状态"}</Badge>
    );
  }
      ...
      <div className="space-y-6 p-4 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              统一汇总平台用户订单，统计、列表与详情均来自商城唯一订单事实源。
            </p>
          </div>
          <Button
            variant="outline"
            onClick={() =>
              void Promise.all([query.refetch(), statsQuery.refetch()])
            }
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Card>
            <CardContent className="flex items-center justify-between p-5">
              <div>
                <p className="text-sm text-muted-foreground">近7日订单</p>
                <p className="mt-2 text-2xl font-semibold">
                  {stats?.sevenDayOrders ?? "—"}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  最近7×24小时
                </p>
          ...
          <Card>
            <CardContent className="flex items-center justify-between p-5">
              <div>
                <p className="text-sm text-muted-foreground">交易用户</p>
                <p className="mt-2 text-2xl font-semibold">
                  {stats?.buyerCount ?? "—"}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  供应商 {stats?.sellerCount ?? "—"} 家
                </p>
              </div>
          ...
          <CardContent className="flex flex-wrap items-center gap-2 p-4">
            ...
            {Object.entries(statusMeta).map(([value, meta]) => (
              <button
                key={value}
                type="button"
                className={`rounded-full border px-3 py-1.5 text-sm transition-colors hover:border-primary ${status === value ? "border-primary bg-primary/5" : "border-border"}`}
                onClick={() => {
                  setStatus(value as keyof typeof statusMeta);
                  setPage(1);
                }}
              >
                {meta.label}{" "}
                <span className="font-semibold">
                  {stats?.statusCounts[value as keyof typeof statusMeta] ?? "—"}
                </span>
              </button>
            ))}
            {statsQuery.error && (
              <span className="text-sm text-destructive">
                统计暂不可用：{statsQuery.error.message}
              </span>
            )}
          </CardContent>
        </Card>
                      ...
                      >
                        <TableCell>
                          ...
                          <div className="mt-1 text-xs text-muted-foreground">
                            {order.batchNo
                              ? `支付批次 ${order.batchNo} · 子单 ${String(order.batchSeq).padStart(2, "0")}`
                              : "独立 DZ 订单"}
                          </div>
                        </TableCell>
                        {/*
                          ...
                          <div className="mt-1 text-xs text-muted-foreground">
                            ...
                            {order.buyerCompanyName
                              ? order.buyerName ||
                                ...
                                `用户 ${order.buyerId}`
                              : "未填企业资料"}
                            {order.buyerSalesOwner
                              ? ` · 销售 ${order.buyerSalesOwner}`
                              : ""}
                          </div>
                        </TableCell>
      ...
      <div className="space-y-6 p-4 sm:p-6">
        <div>
          ...
          <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {order.batchNo
                  ? `支付批次 ${order.batchNo} · 第 ${String(order.batchSeq).padStart(2, "0")} 张子订单`
                  : "独立 DZ 订单（无支付批次）"}
              </p>
            </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {order.buyerCompanyName ? (
                <p className="text-muted-foreground">
                  联系人：{order.buyerName || order.buyerUsername || "—"}
                </p>
              ) : null}
              {order.buyerSalesOwner ? (
                <p className="text-muted-foreground">
                  归属销售：{order.buyerSalesOwner}
                </p>
              ) : null}
              <p>
                供应商：{order.sellerName}{" "}
                <span className="text-muted-foreground">
                  （用户ID {order.sellerId}）
                </span>
              </p>
              <p>支付方式：{payMethodLabels[order.payMethod]}</p>
              <p>
                未税 {money(order.amountEx)} · 税额 {money(order.taxAmount)} ·
                运费 {money(order.shippingFee)}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
              <CardTitle className="text-base">收货与物流</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2 text-sm">
              <p>
              ...
              <p>
                {order.expressCo && order.expressNo
                  ? `${order.expressCo} · ${order.expressNo}`
                  : "尚未发货"}
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <p>{order.note || "无买家备注"}</p>
              {order.statusNote && (
                <p className="text-muted-foreground">
                  状态说明：{order.statusNote}
                </p>
              )}
            </CardContent>
          </Card>
        ...
        {data.siblings.length > 1 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">同一父订单的子订单</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-2 md:grid-cols-2">
              {data.siblings.map(item => (
                <Link key={item.id} href={`/orders/${item.id}`}>
                  <div
                    className={`rounded-lg border p-3 transition-colors hover:bg-muted ${item.id === orderId ? "border-primary bg-primary/5" : ""}`}
                  >
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">商品项</CardTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">状态轨迹</CardTitle>
          </CardHeader>
          <CardContent>
            {data.tracks.length ? (
